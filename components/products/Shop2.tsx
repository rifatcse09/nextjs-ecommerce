...
export default function Shop2({
  ...
  const [state, dispatch] = useReducer(reducer, {
    ...initialState,
    filtered: electronicsHoverVideoData2,
    sorted: electronicsHoverVideoData2,
  });
  useEffect(() => {
    dispatch({ type: "FILTER_PRODUCTS", payload: electronicsHoverVideoData2 });
  }, [
    state.brands,
    state.categories,
    state.colors,
  ...
  function getFilterCount(
    filterFunction: (
      product: (typeof electronicsHoverVideoData2)[0],
    ) => boolean,
  ) {
    return electronicsHoverVideoData2.filter((product) =>
